...
// 龙族肾上腺素物品类
public class DragonAdrenalineItem extends Item {
    private static final Logger LOGGER = LogManager.getLogger();
    private static final Random RANDOM = new Random();

    public DragonAdrenalineItem(Properties properties) {
        super(properties);
    }
    ...
    private void startRevivalProcess(EntityDragonBase dragon, ServerLevel serverLevel, Player player) {
        ...
        new Thread(() -> {
            try {
                int revivalTime = dragon.getAgeInDays() * 100; // 复活时间 = 龙生长天数 * 100
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> tooltip, TooltipFlag flag) {
        tooltip.add((Component.translatable("item.dragonreborn.dragon_adrenaline.tooltip")).withStyle(ChatFormatting.GRAY));
    }
}